...
class AdminVesselsNoonRepArchive extends React.Component {
  ...
  getArchives = () => {
    const requestURL = general.getAPIUrl() + "/email/noon/report";
  ...
  getVesselReport = () => {
    const requestURL = general.getAPIUrl() + "/email/noon/report/data";
    this.toggleLoading(true);
  ...
  downloadVesselReports = type => {
    const requestURL = general.getAPIUrl() + "/email/noon/report/" + type;
    this.toggleLoading(true);
    ...
    request(requestURL, {
      ...
      .then(response => {
        if (!this.props.handleRequest(response)) return;
        const url = general.getAPIUrl() + "/" + response.location;
            ...
            <Card>
              ...
              <CardBody className={classes.centerText}>
                ...
                <Button
                  ...
                  onClick={() => this.downloadVesselReports("pdf")}
                  color="info"
                >
                  Download Noon Report(s) PDF
                </Button>
                <Button
                  disabled={this.state.selectedArchiveId.length === 0}
                  onClick={() => this.downloadVesselReports("csv")}
                  color="info"
                >
                  Download Noon Report(s) CSV
                </Button>
              </CardBody>
            </Card>
          </GridItem>
