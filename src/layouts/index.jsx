...
class Dashboard extends React.Component {
  ...
  sidebarFilter = val => {
    ...
    if(val){
      if(val.value === "all"){
        ...
        let temp = (stateCopy.searchBox.suggestions).filter(obj => {
          return ((obj.update_state).toLowerCase()) === ((val.value).toLowerCase());
        })
        if(stateCopy.searchBox.selectedVesselCompany !== "All"){
          temp = temp.filter(obj => {
            const comps = obj.company;
            if(stateCopy.searchBox.selectedVesselCompany === "Others"){
              return (comps.length === 0 );
            } else {
              return (comps.find(comp => comp.company_name === stateCopy.searchBox.selectedVesselCompany) !== undefined);
            }
          })
        }
  ...
  sidebarFilterCompany = val => {
      ...
      if(val.value === "All"){
        ...
        let temp = (stateCopy.searchBox.suggestions).filter(obj => {
          ...
          if(val.value === "Others"){
            ...
            return (comps.find(comp => comp.company_name === val.value) !== undefined);
          }
        })
    ...
    stateCopy.searchBox.selectedVesselCompany = val.value;
  ...
  updateCompanyList = companies => {
    let stateCopy = Object.assign({}, this.state);
    stateCopy.searchBox.companies = companies;
    this.setState(stateCopy);
  }
    ...
    return (
      <div className={classes.wrapper}>
        <Sidebar
          ...
          updateCompanyList={this.updateCompanyList}
          searchSuggestions={this.state.searchBox.suggestions}
          companyList={this.state.searchBox.companies}
          selectedSuggestions={this.state.searchBox.selectedSuggestions}
          sidebarFilter={this.sidebarFilter}
          sidebarFilterCompany={this.sidebarFilterCompany}
          selectedVesselState={this.state.searchBox.selectedVesselState}
          {...rest}
        />
