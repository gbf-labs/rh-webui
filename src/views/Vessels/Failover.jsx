...
class Failover extends React.Component {
  state = {
    //table
    loading: false,
    loading2: false,
  ...
  updateVesselState(combined) {
    const requestURL = general.getAPIUrl() + "/graph/selected";
    let selectedGraphs = [];
  ...
  getChartOptions() {
    const requestURL = general.getAPIUrl() + "/graph";
  ...
  getCharts(combine) {
    ...
    let selectedGraphs = [];
    let requestURL = general.getAPIUrl() + "/graph";
    if (combined) {
      for (let i = 0; i < this.state.selectedGraphNameListCombine.length; i++) {
        const sel = this.state.selectedGraphNameListCombine[i];
        selectedGraphs.push(sel.value);
      }
      requestURL = general.getAPIUrl() + "/graph/combine";
    } else {
      for (let i = 0; i < this.state.selectedGraphNameList.length; i++) {
        const sel = this.state.selectedGraphNameList[i];
        selectedGraphs.push(sel.value);
  ...
  getFailover() {
    const requestURL = general.getAPIUrl() + "/vessels/failover";
    const vessId = this.props.match.params.vessel_id;
  ...
  getPortForwading() {
    const requestURL =
      general.getAPIUrl() + "/device/port/forwarding";
    const vessId = this.props.match.params.vessel_id;
    request(requestURL, {
      method: "GET",
      params: {
                ...
                <div>
                  <Table className={classes.tableLayout}>
                    ...
                    <TableBody>
                      ...
                      {!this.state.loading &&
                        this.state.failoverData.map(row => (
                      ...
                      {!this.state.loading &&
                        this.state.failoverData.length === 0 && (
                          <TableRow>
...
export default withStyles(styles)(withSnackbar(Failover));
